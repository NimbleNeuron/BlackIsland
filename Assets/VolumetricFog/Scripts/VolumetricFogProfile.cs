...
namespace VolumetricFogAndMist
{
	...
	public class VolumetricFogProfile : ScriptableObject
	{
		...
		[Range(0, 1.25f)] public float density = 1.0f;

		[Range(0, 1f)] public float noiseStrength = 0.8f;

		[Range(0, 500)] public float height = 4f;

		[Range(0, 1f)] public float heightFallOff = 0.6f;

		public float baselineHeight = 0f;

		[Range(0, 1000)] public float distance = 0f;

		[Range(0, 5f)] public float distanceFallOff = 0f;

		public float maxFogLength = 1000f;

		[Range(0, 1f)] public float maxFogLengthFallOff = 0f;

		public bool baselineRelativeToCamera = false;

		[Range(0, 1f)] public float baselineRelativeToCameraDelay = 0;

		[Range(0.2f, 10f)] public float noiseScale = 1f;

		[Range(-0.3f, 2f)] public float noiseSparse = 0f;

		[Range(1f, 2f)] public float noiseFinalMultiplier = 1f;

		[Range(0, 1.05f)] public float alpha = 1f;

		public Color color = new Color(0.89f, 0.89f, 0.89f, 1);

		[Range(0, 1f)] public float deepObscurance = 1f;

		public Color specularColor = new Color(1, 1, 0.8f, 1);

		[Range(0, 1f)] public float specularThreshold = 0.6f;

		[Range(0, 1f)] public float specularIntensity = 0.2f;

		public Vector3 lightDirection = new Vector3(1, 0, -1);

		[Range(-1f, 3f)] public float lightIntensity = 0.2f;

		public Color lightColor = Color.white;

		[Range(0, 1f)] public float speed = 0.01f;

		public bool useRealTime;

		public Vector3 windDirection = new Vector3(-1, 0, 0);

		[Range(0, 10f)] public float turbulenceStrength = 0f;
		...
		[Range(0, 1f)] public float skySpeed = 0.3f;

		[Range(0, 1f)] public float skyNoiseStrength = 0.1f;

		public float skyNoiseScale = 1.5f;

		[Range(0, 1f)] public float skyAlpha = 1f;

		[SerializeField] [Range(0, 0.999f)] private float skyDepth = 0.999f;
		...
		[Range(0.00001f, 0.005f)] public float edgeThreshold = 0.0005f;
		...
		/// </summary>
		public static void Lerp(VolumetricFogProfile profile1, VolumetricFogProfile profile2, float t,
			VolumetricFog fog)
		{
			if (t < 0)
			{
			...
			fog.baselineRelativeToCamera =
				t < 0.5f ? profile1.baselineRelativeToCamera : profile2.baselineRelativeToCamera;
			fog.baselineRelativeToCameraDelay = profile1.baselineRelativeToCameraDelay * (1f - t) +
			                                    profile2.baselineRelativeToCameraDelay * t;
			...
			if (profile1.lightScatteringOverride && profile2.lightScatteringOverride)
			{
				...
				fog.lightScatteringDiffusion = profile1.lightScatteringDiffusion * (1f - t) +
				                               profile2.lightScatteringDiffusion * t;
				fog.lightScatteringExposure =
					profile1.lightScatteringExposure * (1f - t) + profile2.lightScatteringExposure * t;
				fog.lightScatteringIllumination = profile1.lightScatteringIllumination * (1f - t) +
				                                  profile2.lightScatteringIllumination * t;
				fog.lightScatteringJittering = profile1.lightScatteringJittering * (1f - t) +
				                               profile2.lightScatteringJittering * t;
				fog.lightScatteringSamples =
					(int) (profile1.lightScatteringSamples * (1f - t) + profile2.lightScatteringSamples * t);
				fog.lightScatteringSpread =
					profile1.lightScatteringSpread * (1f - t) + profile2.lightScatteringSpread * t;
				fog.lightScatteringWeight =
					profile1.lightScatteringWeight * (1f - t) + profile2.lightScatteringWeight * t;
			}
		}
