...
namespace MLSpace
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
		...
		[HideInInspector] public bool simulateRootMotion = true;
		...
		private void ScaleCapsuleForCrouching(bool crouch)
		{
			...
			else
			{
				...
				if (Physics.SphereCast(ray, m_Capsule.radius * 0.5f, out raycastHit, maxDistance, colliderLayer.value))
				{
					m_Crouching = true;
					return;
				}
		...
		private void UpdateAnimator(Vector3 move)
		{
			...
			float value =
				(Mathf.Repeat(m_Animator.GetCurrentAnimatorStateInfo(0).normalizedTime + m_RunCycleLegOffset, 1f) < 0.5f
					? 1
					: -1) * m_ForwardAmount;
			if (m_IsGrounded)
			{
		...
		private void ApplyExtraTurnRotation()
		{
			...
			transform.Rotate(0f, m_TurnAmount * num * Time.deltaTime, 0f);
		}
		...
		{
			RaycastHit raycastHit;
			if (Physics.Raycast(transform.position + Vector3.up * 0.1f, Vector3.down, out raycastHit,
				m_GroundCheckDistance, colliderLayer.value))
			{
